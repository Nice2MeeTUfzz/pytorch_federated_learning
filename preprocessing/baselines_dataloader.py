...
def divide_data_noiid(num_client=1, num_local_class=10, dataset_name='emnist', i_seed=0):
    ...
    # print(json.dumps(config_class, indent=4))
    # print(json.dumps(config_division, indent=4))
...
def divide_data_iid(num_client=1, dataset_name="MNIST", i_seed=0):
    torch.manual_seed(i_seed)
    trainset, testset, len_classes = load_data(dataset_name, download=True, save_pre_data=False)
    ...
    all_indices = all_indices[torch.randperm(len(all_indices))].tolist()

    # Evenly distribute the shuffled indices across all clients
    indices_per_client = len(all_indices) // num_client
    remainder = len(all_indices) % num_client

    for i in range(num_client):
        start_idx = i * indices_per_client + min(i, remainder)
        end_idx = (i + 1) * indices_per_client + min(i + 1, remainder)
        client_indices = all_indices[start_idx:end_idx]

        # Create a subset for this client using the selected indices
        user_data = Subset(trainset, client_indices)

        # Update configurations
        trainset_config['users'].append(f'f_{i:05d}')
        trainset_config['user_data'][f'f_{i:05d}'] = user_data
...
if __name__ == "__main__":
    ...
    tc, _ = divide_data_iid(num_client=11, dataset_name="MNIST", i_seed=1)
    print(tc)
